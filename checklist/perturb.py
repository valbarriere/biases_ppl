"""
Change the function to change the names into names of different countries 

Add change_names_country_specific()

Need to add 2 jsons in the data/ folder of the library
...
def load_data():
	...
	if BOOL_USE_CHECKLIST_NAMES:
		...
		dict_first_names_EU = json.load(open(os.path.join(cur_folder, 'data', 'firstnames_EU.json')))
		dict_last_names_EU = json.load(open(os.path.join(cur_folder, 'data', 'lastnames_EU.json')))

		list_target_countries = []
		for target_country in dict_first_names_EU.keys():
			if target_country in dict_last_names_EU.keys():
				list_target_countries.append(target_country)

		# concat dictionnaries 
		for target_country, v in dict_first_names_EU.items():
			if target_country in list_target_countries:
				data['name'][target_country] = v
		for target_country, v in dict_last_names_EU.items():
			if target_country in list_target_countries:
				data['name'][target_country]['last'] = v
...
class Perturb:
	...
	def _change_names_country_specific(doc, target_country='France', target_gender='male', meta=False, n=10, first_only=False, last_only=False, seed=None):
		...
		for x in ents:
			...
			sex = None
			if f.capitalize() in Perturb.data['name_set']['women']:
			# if f.capitalize() in (Perturb.data['name_set'][target_country]['women'] + Perturb.data['name_set']['women']):
				sex = 'women'
			if f.capitalize() in Perturb.data['name_set']['men']:
			# if f.capitalize() in (Perturb.data['name_set'][target_country]['men'] + Perturb.data['name_set']['men']):
				sex = 'men'
			if not sex:
				continue
			...
			if len(x.split()) > 1:
				...
				if len(l) > 2 and l.capitalize() not in Perturb.data['name_set']['last']:
				# if len(l) > 2 and l.capitalize() not in Perturb.data['name_set'][target_country]['last']: # Pas besoin de ca, 
					continue
			...
			# if a single name only (can be first of last)
				...
				if last_only:
					return None
			# names = Perturb.data['name'][sex][:90+n] # 90+n ????? 
			...
			if not first_only:
				...
				if len(x.split()) > 1:
					# last = Perturb.data['name']['last'][:90+n] # 90+n ????? 
					last = Perturb.data['name'][target_country]['last'][target_gender] # target gender because for russian female and male names are different
					last = np.random.choice(last, n)
